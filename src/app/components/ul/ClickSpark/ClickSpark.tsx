...
const ClickSpark: React.FC<ClickSparkProps> = ({
  ...
  useEffect(() => {
    if (!isClient) return; // Skip rendering until the client side is ready
  ...
  useEffect(() => {
    if (!isClient) return; // Skip if not in client-side rendering
  ...
  }, [isClient, sparkColor, sparkSize, sparkRadius, sparkCount, duration, easeFunc, extraScale]);
  ...
  if (!isClient) {
    return null; // Render nothing on SSR to avoid hydration mismatch
  }

  return (
    <div className="relative w-full h-full pointer-events-auto" onClick={handleClick}>
      <canvas
        ref={canvasRef}
